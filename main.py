...
def main():
    # get game name, tag line & region of victim
    if len(sys.argv) < 3:
        game_name = str(input("Game Name: "))
        tag_line = str(input("Tag Line: "))
    ...
    print('[Input] Riot ID: ' + game_name + '#' + tag_line)
    ...
    for app_port, auth_token in g_auths.items():
        if g_regions[app_port] == region:
            ...
            # api urls
            alias_lookup = api + '/lol-summoner/v1/alias/lookup?gameName=' + game_name + '&tagLine=' + tag_line
            summoner_by_puuid = api + '/lol-summoner/v2/summoners/puuid/'
            ...
            # get summoner id
            if (summoner_id == None):
                r = requests.get(alias_lookup, headers=headers, verify=False)
                r = json.loads(r.text)
                # print(r)
                summoner_puuid = r['puuid']
                print('Summoner PUUID: ' + str(summoner_puuid))
                r = requests.get(summoner_by_puuid + summoner_puuid, headers=headers, verify=False)
                r = json.loads(r.text)
